		...
		if ($reachgrist == True) {
	      ...
	      if ($field == "Echeladder") {
		    if ($targetrow['Echeladder'] == 1) {
              mysql_query("UPDATE Players SET `Gel_Viscosity` = 10 WHERE `Players`.`username` = '" . $_POST['user'] . "' LIMIT 1;");
            } elseif ($targetrow['Echeladder'] == 2) {
              mysql_query("UPDATE Players SET `Gel_Viscosity` = 15 WHERE `Players`.`username` = '" . $_POST['user'] . "' LIMIT 1;");
            } elseif ($targetrow['Echeladder'] == 3 || $targetrow['Echeladder'] == 4 || $targetrow['Echeladder'] == 5) {
              mysql_query("UPDATE Players SET `Gel_Viscosity` = " . strval($targetrow['Echeladder']*10-5) . " WHERE `Players`.`username` = '" . $_POST['user'] . "' LIMIT 1;");
            } else {
              mysql_query("UPDATE Players SET `Gel_Viscosity` = " . strval($targetrow['Echeladder']*15-25) . " WHERE `Players`.`username` = '" . $_POST['user'] . "' LIMIT 1;");  
		}
