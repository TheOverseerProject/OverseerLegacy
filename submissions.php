...
echo "<!DOCTYPE html><html><head><style>itemcode{font-family:'Courier New'}</style><style>normal{color: #111111;}</style><style>urgent{color: #0000CC;}</style><style>defunct{color: #CC0000;}</style><style>clarify{color: #CCCC00;}</style><style>greenlit{color: #00AA00;}</style><style>suspended{color: #999999;}</style><style>randomized{color: #EE6606;}</style><style>halp{color: #FFFFFF;}</style></head><body>";
  ...
  if (!empty($_POST['delete'])) {
    ...
    if ($feedrow['user'] == $username || $userrow['session_name'] == "Developers" || $userrow['modlevel'] >= 3) {
      ...
      } else echo "You can't delete non-item submissions from here.</br>";
  ...
  if (!empty($_POST['moderatethis'])) {
    ...
    if ($userrow['modlevel'] >= 1) {
      ...
      } else echo "You can't moderate a non-item.</br>";
  ...
  if (!empty($_GET['view'])) {
	...
	if (!empty($_POST['body'])) {
	  ...
	  $exstring = ": ";
	  if ($userrow['modlevel'] > 0) $exstring = " (Level " . strval($userrow['modlevel']) . " Mod): ";
	  if ($userrow['session_name'] == "Developers") $exstring = " (Developer): ";
	  if ($userrow['session_name'] == "Itemods") $exstring = " (G. Moderator): ";
	...
	if ($userrow['modlevel'] >= 1) {
		...
		if ($userrow['modlevel'] >= 3) echo ' - Also grant encounters: <input type="text" name="greenenc" /></br>';
		...
		if ($userrow['modlevel'] >= 2) echo '<input type="radio" name="modaction" value="clear"> <normal>Clear moderative flags</normal></br>';
	...
	if ($feedrow['greenlight'] == 1 && $userrow['modlevel'] >= 4) {
        ...
        } else echo 'The submission with that ID is not an item.</br>';
  ...
  while ($row = mysql_fetch_array($countresult)) {
  	...
  	if ($pcount == 20) {
  		...
  		if ($ptotal == $page) {
  			...
  			echo '<a href="submissions.php?page=' . strval($ptotal) . '&sort=' . $sort . '&mode=' . $mode . '&order=' . $order . '&search=' . $_GET['search'] . '&sfield=' . $sfield . '">' . strval($ptotal) . '</a> | ';
  		}
  		$pcount = 0;
  	}
  	$pcount++;
  ...
  if ($page < $ptotal) {
    echo '<a href="submissions.php?page=' . strval($page + 1) . '&sort=' . $sort . '&mode=' . $mode . '&order=' . $order . '&search=' . $_GET['search'] . '&sfield=' . $sfield . '">Next page</a>';
  } else {
  	echo 'Next page';
  }
  ...
  if ($userrow['modlevel'] > 0) {
  	...
  	if ($userrow['modlevel'] >= 1) echo "- You may apply the five moderative flags (red, yellow, green, gray, white) to any unmarked submission.<br />";
  	if ($userrow['modlevel'] >= 2) echo "- You may overwrite an existing flag or clear all flags on a submission.<br />";
  	if ($userrow['modlevel'] >= 3) echo "- You may delete red-flagged subs and grant encounters for greenlit items.<br />";
  	if ($userrow['modlevel'] >= 4) echo "- You may use the Item Editor to add greenlit items to the game. Look for the 'Take this to the Item Editor' button when viewing a greenlit item.<br />";
  	if ($userrow['modlevel'] >= 5) echo "- You may freely use the <a href='devtools/itemedit.php'>Item Editor</a> to edit items or create items from scratch. You may also use the <a href='devtools/consumedit.php'>Consumable Editor</a>.<br />";
  	if ($userrow['modlevel'] >= 6) echo "- You can view the working item addlog and may post item updates from the Item/Consumable Editor.<br />";
  	if ($userrow['modlevel'] >= 10) echo "- You have the powers of a Global Moderator and can access <a href='devtools'>a variety of tools</a>.<br />";
