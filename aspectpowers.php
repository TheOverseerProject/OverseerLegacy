...
if (empty($_SESSION['username'])) {
  ...
  if (!empty($_POST['usepattern'])) { //User accessing a pattern.
	...
	if ($userrow['enemydata'] != "" || $userrow['aiding'] != "") { //Player strifing.
	  ...
	  if ($cost < 20) $cost = 20; //No ability may cost less than 20% of the aspect vial.
...
AND `Abilities`.`Rungreq` BETWEEN $userrow[Echeladder]+1 AND $userrow[Echeladder]+$rungs AND `Abilities`.`Godtierreq` = 0 ORDER BY `Abilities`.`Rungreq` DESC;");
		...
		if ($striferow[$enemystr] != "") {
		  $enemyresult = mysql_query("SELECT * FROM Enemy_Types WHERE `Enemy_Types`.`basename` = '" . $striferow[$enemystr] . "'");
		  ...
		  if (!empty($abilities[28])) { //the Witch roletech Curse activates, providing a chance for the damaging/power reducing patterns to inflict a status
		  	if ($j == 1) echo $abilities[28] . "<br />"; //just echo this once
		  	$statuschance = $patternrow[$damagestr] + $patternrow[$powerdownstr];
		  	if ($statuschance == 100) $statuschance = 150; //make it almost certain to trigger at all times if the user poured all their points into it
		  	switch ($userrow['Aspect']) {
		  		case 'Time': $bonuseffects = "TIMESTOP:$statuschance|"; break;
		  		case 'Space': $bonuseffects = "SHRINK:$statuschance|"; break;
		  		case 'Breath': $bonuseffects = "KNOCKDOWN:$statuschance|"; break;
		  		case 'Light': $bonuseffects = "MISFORTUNE:$statuschance|"; break;
		  		//mind can inflict both, since these effects are rather weak on their own
		  		case 'Mind': $bonuseffects = "DISORIENTED:$statuschance|DISTRACTED:$statuschance|"; break;
		  		case 'Heart': $bonuseffects = "LOCKDOWN:$statuschance|"; break;
		  		case 'Life': $bonuseffects = "WATERYGEL:$statuschance|"; break;
		  		//might change the severity of this later
		  		case 'Doom': $bonuseffects = "POISON:$statuschance:2.5|"; break;
		  		case 'Void': $bonuseffects = "GLITCHED:$statuschance|"; break;
		  		case 'Rage': $bonuseffects = "ENRAGED:$statuschance|"; break;
		  		case 'Hope': $bonuseffects = "HOPELESS:$statuschance|"; break;
		  		case 'Blood': $bonuseffects = "BLEEDING:$statuschance|"; break;
		  	}
		  }
		...
		$i++;
	      ...
	      mysql_query("UPDATE `Players` SET `strifestatus` = '$currentstatus' WHERE `Players`.`username` = '" . $striferow['username'] . "'");
	    }
	    $powerboost = 0;
	    $temp = False;
	    ...
	    if ($patternrow[$healstr] != 0) {
		  ...
		  if ($patternrow[$healstr] < 0) $patternrow[$healstr] = 0; //Cannot injure fellow players.
  ...
  while ($slot <= $max_patterns) {
    ...
    if (!empty($patternrow[$namestr])) {
      ...
      if (!empty($abilities[28])) {
      	$statuschance = $patternrow[$damagestr] + $patternrow[$powerdownstr];
      	if ($statuschance == 100) $statuschance = 150;
      	if ($statuschance > 0) {
      	switch ($userrow['Aspect']) {
		  		case 'Time': $effstr = "Timestop"; break;
		  		case 'Space': $effstr = "Shrinking"; break;
		  		case 'Breath': $effstr = "Knockdown"; break;
		  		case 'Light': $effstr = "Misfortune"; break;
		  		case 'Mind': $effstr = "Disoriented and/or Distracted"; break;
		  		case 'Heart': $effstr = "Lockdown"; break;
		  		case 'Life': $effstr = "Waterygel"; break;
		  		case 'Doom': $effstr = "Poison"; break;
		  		case 'Void': $effstr = "Glitched Out"; break;
		  		case 'Rage': $effstr = "Enraged"; break;
		  		case 'Hope': $effstr = "Hopeless"; break;
		  		case 'Blood': $effstr = "Bleeding"; break;
		  	}
	...
	$cost = floor($cost * 0.85);
      ...
      if ($cost < 20) $cost = 20; //No ability may cost less than 10% of the aspect vial.
      echo "Cost: $cost%</br>";
